...
function search() {
   // Array_Video = JSON.parse(localStorage.getItem('Array_Video'));
   Array_Video.shift();
   // Array_Video = []; // xóa phần tử đầu tiên trong mảng
   fetch(url + parameter + '&q=' + KeyWord.value + '&key=' + apiKey)
      ...
      .then(function (data) {
         ...
         localStorage.setItem('Array_Video', JSON.stringify(Array_Video));
