...
function Video_Active() {
    let Video_List = document.querySelectorAll(".container-body-content");
    let Full_img = document.querySelectorAll(".container-body-content-image img");
    let Length = Video_List.length;
    for (let i = 0; i < Length; i++) {
        if(Full_img[i].classList.contains("active")){
            Full_img[i].classList.remove("active");
        }
        if (Video_List[i].getAttribute("index") == Index) {
            Full_img[i].classList.add("active");
        }
        Video_List[i].setAttribute("onclick", "clickeds(this)");
    }
};
function clickeds(element){
    let getIndex = element.getAttribute("index");
    Index = getIndex;
    Video_Active();
}