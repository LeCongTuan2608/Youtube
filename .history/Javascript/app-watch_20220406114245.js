...
window.addEventListener('load', () => {
   //    Active_like_dislike();
   array_comments.map((commented) => {
      return Commented_Text_Container.insertAdjacentHTML('beforebegin', output_commented);
   });
});
      ...
      let output_commented = `
         <div class="commented-container">
            ...
            <div class="commented-content">
               <h4>Tuấn Lê</h4>
               <span>${Comment_Text.value}</span>
            </div>
         </div>`;
      Commented_Text_Container.insertAdjacentHTML('beforebegin', output_commented);
   }
   array_comments.push(Commented_Text_Container);
   Comment_Text.value = '';
});
