...
function Active_Video() {
   for (let j = 0; j < a_id.length; j++) {
      if (a_id[j].getAttribute('index_s') == Index) {
         // a_id[j].classList.add("is_playing");
         console.log('show: đã click');
      }
      // a_id[j].setAttribute('onclick', 'active(this)');
   }
}
let a_id = document.querySelectorAll('.container-body-content a');
a_id[j].addEventListener('click', function () {
   let getIndex = a_id.getAttribute('index');
   Index = getIndex;
   console.log('show: đã click');
});
// function actice(element) {
//    let getIndex = element.getAttribute('index');
//    Index = getIndex;
//    Active_Video();
// } //end
