...
const a_id = document.querySelectorAll('.container-body-content a');
function active() {
   let Content = document.querySelectorAll('.container-body-content');
   let Length_Content = Content.length;
   for (let t = 0; t < Length_Content; t++) {
      let getIndex = a_id[t].getAttribute('index_s');

      Index = getIndex;
      Content[t].addEventListener('click', function () {
         if (a_id[t].classList.getAttribute('index_s') == Index) {
            console.log('show: 10');
         }
      });
   }
