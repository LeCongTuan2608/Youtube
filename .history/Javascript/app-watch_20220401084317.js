...
function Result_List_watch() {
   ...
   for (let i = 0; i < Length_Video; i++) {
      ...
      console.log(Contents_video);
      // let Contents_Video = document.querySelectorAll('.ytb-contents-container ');
      let Contents_id = document.querySelectorAll('.ytb-contents-container a');
      for (let j = 0; j < Length_Video; j++) {
         Contents_Video[j].onclick = function () {
            let getIndex = Contents_Video[j].getAttribute('number');
            Index = getIndex;
            if (Contents_Video[j].getAttribute('number') == Index) {
