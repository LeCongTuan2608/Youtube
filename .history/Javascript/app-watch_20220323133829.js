import { List_Video } from './app.js';
console.log(Array_Video);
function Result_watch(List_Video) {
   const Contents_video = document.querySelector('.ytb-list-contents');
   let Length_Video = List_Video[0].items.length;
   for (let i = 0; i < Length_Video; i++) {
      let Id_video = List_Video[0].items[i].id.videoId;
      let Thumbnails = List_Video[0].items[i].snippet.thumbnails.medium.url;
      let Title = List_Video[0].items[i].snippet.title;
      let output = `
